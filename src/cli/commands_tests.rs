...
fn cli_auto_poke_cooldown_blocks_within_60s() {
    ...
    let mut last_poke = Some(std::time::Instant::now() - std::time::Duration::from_secs(30));
    assert!(should_skip_auto_poke_for_swarm(
        "test-session",
        &mut last_poke
    ));
}
...
fn cli_auto_poke_cooldown_allows_after_60s() {
    ...
    let mut last_poke = Some(std::time::Instant::now() - std::time::Duration::from_secs(90));
    assert!(!should_skip_auto_poke_for_swarm(
        "test-session",
        &mut last_poke
    ));
