...
fn test_loading_preview_refreshes_search_index_for_picker_filtering() {
    ...
    let mut session = Session::create_with_id(
        ...
        // No parent: this fixture stands in for a session the user opened, and
        // child sessions are (correctly) hidden from the picker.
        None,
...
// ---------------------------------------------------------------------------
// Machine-created sessions must stay out of the user's resume list.
//
// Regression cover for the flood that made /resume unusable: every spawned
// agent wrote a session, and because the picker only filtered `is_debug`,
// swarm workers, cheap-route subtasks and one-shot runs all listed as if the
// user had opened them. These tests pin the *rule*, not one spawn path, so a
// future spawn site cannot quietly reintroduce the flood.
// ---------------------------------------------------------------------------
