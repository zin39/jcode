//! Tiered semantic palette: 12 roles mapped to three color-precision tiers.
//!
//! This module is the fork's tier-aware color system (TUI redesign spec
//! §2.1). Each role maps to a specific truecolor hex, a fixed 256-color
//! palette index, and a named ANSI-16 slot so every tier is a designed
//! experience, not a broken rich screen.
//!
//! It coexists with [`crate::palette`], the user-configurable palette from
//! upstream: `palette` answers "what did the user configure this role to
//! be", while `tiered` answers "how should this semantic role degrade on
//! less capable terminals". Callers that need tier-aware degradation
//! (gutter glyphs, plain-mode fallbacks) use this module.
