...
/// billed every swarm worker to the generic `openai-compatible` profile
/// (dashscope) instead of DeepSeek direct, because `split_once(':')` took
/// `openai-compatible` as the profile id and left `deepseek:deepseek-v4-pro`
/// as the model name. The endpoint is the thing that costs money, so this
/// asserts the resolved base URL rather than only the parsed pair.
#[test]
fn fully_qualified_compatible_pin_resolves_to_its_profile_not_the_generic_endpoint() {
    let (profile, model) = MultiProvider::openai_compatible_model_prefix(
    ...
    // The two-segment form is the canonical spelling and must keep working.
    let (short_profile, short_model) =
        MultiProvider::openai_compatible_model_prefix("deepseek:deepseek-v4-pro")
            .expect("two-segment pin should resolve");
    assert_eq!(short_profile.id, profile.id);
    assert_eq!(short_model, model);

    // Stripping is conditional: a bare transport prefix with no profile
    // segment must not be mistaken for a profile pin.
    assert!(
        MultiProvider::openai_compatible_model_prefix("openai-compatible:glm-5.2").is_none()
            || MultiProvider::openai_compatible_model_prefix("openai-compatible:glm-5.2")
                .is_some_and(|(p, _)| p.id != "deepseek"),
