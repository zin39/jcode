...
fn test_skills_section_truncates_description_to_first_sentence_and_cap() {
    ...
    // Only the first sentence (pre-period) should remain; cap not reached here.
    assert!(
    ...
    // Hard 80-char cap: very long first sentence gets truncated.
...
fn test_skills_section_honors_total_budget_with_overflow_names_only() {
    ...
    // 143 skills (real-world count) with modest descriptions: must fit under 4000 chars.
    ...
    // With 50 skills at this size, overflow should have kicked in.
    assert!(
...
fn test_skills_section_handles_multibyte_utf8_without_panic() {
    ...
    // First sentence is all multi-byte emoji; 80 bytes lands mid-codepoint.
    let description = "🚀".repeat(50); // 200 bytes
    let skills = vec![SkillInfo {
