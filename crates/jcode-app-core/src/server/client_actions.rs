...
/// coordinator's tool choice. Live proposer/aggregate progress streams to the
/// session side panel via `SidePanelDebateReporter`; the synthesized result is
/// streamed into the chat (TextDelta/MessageEnd) and persisted to history.
pub(super) async fn handle_run_gold(
    id: u64,
    task: String,
    ...
    let backend =
        crate::agent::cheap_route::ProviderCheapBackend::new(provider.clone(), registry.clone())
            .with_gold(true, gold_k)
            .with_reporter(reporter);

    let text = match crate::agent::cheap_route::run_cheap_route(&backend, &task).await {
        Ok(outcome) => crate::tool::format_cheap_outcome(&outcome),
        Err(e) => format!(
            "Gold debate failed: {}",
            crate::util::format_error_chain(&e)
        ),
    };

    // Stream the synthesized result into the chat like a normal assistant turn,
    // then persist it to the server-side session history.
    let _ = client_event_tx.send(ServerEvent::TextDelta { text: text.clone() });
    let _ = client_event_tx.send(ServerEvent::MessageEnd);
    {
        let mut agent_guard = agent.lock().await;
