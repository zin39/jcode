use super::{Registry, Tool, ToolContext, ToolOutput, expand_session_tools};
use anyhow::Result;
...
mod tests {
    ...
    // Only the tests read the expanded set back; importing it at module scope
    // would be an unused import in a normal build.
    use crate::tool::session_expanded_tools;
    ...
    fn schema_has_required_names() {
        ...
        assert_eq!(
            names_prop.get("type").and_then(|v| v.as_str()),
            Some("array")
        );
    }
