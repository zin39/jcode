...
///
/// jcode keeps two directories, and the split confused users because it ran
/// along the wrong axis: `*.env` API keys were force-migrated into
/// `app_config_dir()` (see [`migrate_stray_env_files`]) while OAuth token JSON
/// stayed in `jcode_dir()`. Credentials were therefore split *by type*, so
/// "where are my credentials?" had two answers depending on how you logged in.
///
/// The invariant is now one axis, not two:
...
mod secret_path_tests {
    use super::{legacy_secret_path, resolve_secret_path, scoped_test_home, secret_path};
